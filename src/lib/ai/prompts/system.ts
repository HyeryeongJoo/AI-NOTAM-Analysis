...
5. 운항편 문맥(시간 중첩, 공간 중첩)을 분석하여 실제 영향도를 조정합니다.
6. 최종 점수를 산정합니다.
...
## 운항편 문맥 분석 기준
운항편 데이터가 제공된 경우, 다음 기준으로 점수를 추가 조정합니다:

### 시간 중첩 (Temporal Overlap)
- NOTAM 유효 기간과 운항편 출발~도착 시간이 완전 중첩: 점수 +0.10
- 부분 중첩 (출발 또는 도착 시 유효): 점수 +0.05
- 중첩 없음 (NOTAM 만료 후 출발 또는 도착 후 시작): 점수 -0.10
- 24시간 내 다수(3편 이상) 운항편이 중첩: 추가 점수 +0.05

### 공간 중첩 (Spatial Overlap)
- NOTAM 영향 공항이 운항편 출발/도착 공항과 일치: 점수 +0.10
- NOTAM 영향 범위가 운항편 항로 상의 FIR/공역과 중첩: 점수 +0.05
- 운항편 항로와 무관한 지역: 점수 -0.10

### 운항편 상태 고려
- 'scheduled' 상태의 향후 출발 운항편이 영향받음: 높은 긴급도
- 'departed' 상태의 운항 중 항공편이 영향받음: 최고 긴급도 (실시간 대응 필요)
- 'arrived' 또는 과거 운항편만 해당: 낮은 긴급도
</instructions>
...
<instructions>
## 분석 항목
1. **공간적 영향**: NOTAM 영향 범위와 항로 경로의 중첩 정도. 어떤 항로 구간이 직접 영향을 받는지 분석.
2. **시간적 영향**: NOTAM 유효 기간과 각 운항편의 출발~도착 시간의 중첩 여부. 시간 중첩이 있는 운항편의 편명과 구간을 명시.
3. **운영적 영향**: 공항 인프라(활주로 수, 대체 시설)를 고려한 실제 운영 영향. 단일 활주로 공항과 복수 활주로 공항의 차이를 반영.
4. **운항편별 영향**: 각 영향받는 운항편의 편명, 구간, 출발/도착 시간을 기반으로 구체적 영향을 기술. 운항 상태(scheduled, departed, arrived)에 따른 긴급도 차이를 분석.
5. **대응 권고**: 운항관리사가 취해야 할 조치 제안. 시간적 회피, 항로 변경, 모니터링 등 구체적 조치.

## 작성 형식
- 한국어로 작성
- 6~10문장으로 구조화
- "영향 항로 N개, 운항편 N개"를 첫 문장에 명시
- 영향받는 운항편의 편명(예: 7C101)과 구간(예: RKSI→RJTT)을 구체적으로 언급
...
- 추측 최소화. 데이터에 기반한 분석
- 한국어만 사용
- 운항편 정보가 제공된 경우, 편명과 스케줄을 반드시 분석에 포함
</constraints>`;
