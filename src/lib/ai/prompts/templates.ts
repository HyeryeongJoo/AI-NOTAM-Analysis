...
<reference_data>
${qCode ? `Q-Code 설명: ${qCode.descriptionKo} (${qCode.description})
Q-Code 기본 중요도: ${qCode.defaultImportance}` : 'Q-Code 참조 정보 없음'}
${airport ? `공항: ${airport.nameKo} (${airport.name})
ICAO: ${airport.icaoCode}, IATA: ${airport.iataCode}
활주로 수: ${airport.runwayCount}
FIR: ${airport.fir}, 국가: ${airport.country}` : '공항 참조 정보 없음'}
</reference_data>
</notam_data>`;
}
...
export function buildCrewPackageMessage(flight: Flight, notams: Notam[]): string {
  const notamSummary = notams.map((n) => (
    `[${n.importanceLevel.toUpperCase()}] ${n.qCode} at ${n.locationIndicator}: ${n.body}`
  )).join('\n');
...
export function buildRouteAlternativesMessage(
  ...
  const alternateList = alternateRoutes.map((r) => (
